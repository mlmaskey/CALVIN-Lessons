...
#  def add_rech_sinks(self):
#    """
#    added by Mahesh Maskey 5/26/2020, UC Merced.
#    Hack to get rid of surplus water at no cost from recharge regions.
#    Called internally when model is initialized.
#
#    :returns: nothing, but modifies the model object
#    """
#    df = self.df
#    links = df[df.j.str.contains('RECH')].copy(deep=True)
#    links.i = links.j
#    if not links.empty:
#      maxub = links.upper_bound.max()
#      links.j = links.apply(lambda l: 'SINK.'+l.i.split('.')[1], axis=1)
